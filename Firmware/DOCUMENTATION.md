...
- **Velocity excursion flat** — the recent window of velocity stays within `RuntimeConfig::velocityFlatBandMps` of itself.
...
While a phase is open, which window these two tests read is itself two-tiered (v3.11.27, see [below](#the-flat-guards-two-tier-design)): under the absolute ceiling `RuntimeConfig::flatGuardMaxVelocityMps`, both use the dynamic window described above; once the corrected velocity exceeds it, both switch to a fixed, wider window (`RuntimeConfig::velocityOverrideFlatWindowSamples`) instead.
...
### The flat-guard's two-tier design

Even with the live-corrected-estimate fix above, one failure mode remained: during a long stretch with no bracket ever closing, the provisional estimate itself (EMA or rep calibration) can accumulate enough error that the corrected `velZLive` value it produces stays outside `flatGuardMaxVelocityMps` *even while the sensor is genuinely motionless* — not because the sensor is drifting, but because the estimate being used to judge it is wrong, and without a bracket closing, that estimate has no way to correct itself. It's the same underlying failure shape as the raw-vs-corrected fix earlier in this chapter, just moved up one level of indirection.

**v3.11.27 fix — the raw-stillness override almost never fired, and dropping the ceiling wasn't safe either:** through v3.11.26, the safety net for this (`flatGuardOverrideStillTimeS`, 1.0s) watched raw gyroscope/accelerometer stillness instead of `velZLive` — independent of the estimate, but requiring a full, *unbroken* second with every sample in the window under band. User-reported still unresolved on a second real deadlift capture (`STACCO_3`) even after v3.11.26 bounded the drift: the bar's ground contact (unavoidable in a deadlift) is a real, expected deceleration, and `velZ` settled genuinely flat within under a second of it — but the override never fired even once in the whole capture, because post-impact ringdown scatters small noise samples through that window, each one forcing the full second to restart from scratch. A simpler alternative — dropping `flatGuardMaxVelocityMps` from the primary path entirely and trusting the dynamic-window flatness tests alone — was considered and rejected after simulating it against the same capture: at high phase-peak velocity the dynamic window shrinks to as little as 2 samples (20ms), and a smooth velocity curve's acceleration crosses zero at *every* local peak by simple calculus, for a few milliseconds instead of hundreds — with a 2-sample window that momentary crossing at the *top* of a fast rep is indistinguishable from a genuine stop. Confirmed directly in the capture's own first rep: without the ceiling, the engine would have declared "flat" at `t=1.423s` with `velZLive=1.0065 m/s` — the exact peak of a clean 1.27 m/s pull, not a stop. So the ceiling stays on the fast/dynamic-window path (it's load-bearing there), and the override is replaced instead: once a phase is open and `|velZLive|` exceeds `flatGuardMaxVelocityMps`, **both** flatness tests switch from the dynamic window to `RuntimeConfig::velocityOverrideFlatWindowSamples` — a fixed window (30–60 samples, configurable from the app) over the *same* `velZLive`/`worldAccZ` signal the fast path already uses, not raw IMU noise, so an isolated post-impact sample doesn't force a full restart the way the old raw-quiet ring did. Long enough that a genuine pause (flat for hundreds of ms) can't be mistaken for a fast rep's momentary peak (tens of ms), short enough to resolve well inside a second. Verified against both captures before implementing: the wide window correctly does *not* fire at the `STACCO_3` rep-1 peak (actual excursion over the window: 1.01 m/s, nowhere near the 0.08 m/s band it needs), and fires early into the problem stretch that used to run for 8-10 seconds unbounded. Every time it fires it's still flagged in the debug log (`flatGuardOverrideFired`, same field, repurposed) so it can be audited after the fact; `tools/vbt_log_viewer.py` marks these with a red triangle on the overview plot and reports a count in its console summary.
...
| `velocityOverrideFlatWindowSamples` | 45 | Fixed flatness window (samples) used once `\|velZLive\|` exceeds `flatGuardMaxVelocityMps` — replaces the dynamic window for BOTH flatness tests above in that case (v3.11.27, see [The flat-guard's two-tier design](#the-flat-guards-two-tier-design)). Range 30–60. |
...
| `Config` | 38 B, Read/Write | `RuntimeConfig` mirror, not persisted |
